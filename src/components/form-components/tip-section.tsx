...
import SelectExistingTipForm from "@/components/cms/articles/add-edit-articles/form-drawers/select-existing-tip-form";
import { AddButton } from "@/components/ui/add-button";
import { TipsContext } from "@/context/tips-data-provider";
import AddOrEditTip from "@/components/cms/tips/manage-tip";
import { getTips } from "@/lib/services/tips-services";
...
export const TipsSection = ({
  ...
  useEffect(() => {
    const _tipIDs = form.watch("tips") ?? []; // Use watch instead of getValues to avoid stale state
    if (!_tipIDs || _tipIDs.length === 0) return;

    setTipsData((prev: Tip[]) => {
      const existingIds = new Set(prev.map((tip) => tip.id)); // Use Set for fast lookup
      const newTips = _tipIDs
  ...
  }, [form, tips]); // Depend on `tips` to get updates when context changes
