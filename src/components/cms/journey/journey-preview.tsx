...
const JourneyPreviewSection = ({
  ...
  return (
    <div className="relative flex flex-col  w-full h-full ">
      <div
        className={`px-4 ${
          steps != 0 ? "text-white delay-100 duration-100" : "duration-0"
        } transition-colors `}
        style={{
          background:
            steps != 0 ? activeData?.journey["primary-color"] : "transparent",
        }}
      >
        <MobileHeader />
      </div>
      <div
        className={`flex  transition-transform grow max-h-full`}
        style={{
          transform: `translateX(calc(-${steps * 100}% ))`,
        }}
      >
        <div className="overflow-y-auto shrink-0  w-full h-[calc(100%-50px)] no-scrollbar ">
            ...
            <div className="flex justify-between  font-mulish">
              <div className="flex gap-3 text-sm leading-[16.8px]">
                ...
                <div>
                  Good Morning
                  <br />{" "}
                  <span className="font-bold">{auth?.user?.displayName}</span>
                </div>
              </div>
