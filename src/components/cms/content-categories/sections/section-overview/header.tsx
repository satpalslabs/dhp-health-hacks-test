...
const TableHeader = ({
  ...
  table: Table<ArticleTableRow>;
  setSelectedRows: React.Dispatch<React.SetStateAction<ArticleTableRow[]>>;
}) => {
  ...
  //   .filter(
  //     (i) =>
  //       i.collection_data?.sub_section != null &&
  //       i.collection_data?.sub_section != undefined
  //   )
  //   .map((i) => i.collection_data?.sub_section);

  const filterValue = table.getColumn("content_path")?.getFilterValue() as
    | { sub_section?: { id: number }; collection?: { id: number } }
    | undefined;
  // const collections = tableData
  //   .filter((i) =>
  //     selectedSubSection != null
  //       ? i.collection_data?.sub_section === selectedSubSection?.id
  //       : true
  //   )
  //   .map((i) => i.collection)
  //   .filter((i): i is Collection => i != undefined);
  return (
    <Collapsible className="group font-inter">
      <div className="flex items-center justify-between">
              ...
              {/* {subSections.length > 0 && (
                <>
                  <SelectSubSection
                    ...
                    options={subSections.filter((i): i is number => i != null)}
                  />
        ...
        <Button
          onClick={() => {
            ...
            router.push(`/sections/${sectionID}/article/add`);
          }}
        >
      ...
      <div className="flex gap-2">
        ...
        {selectedSubSection && filterValue?.sub_section && (
          <Button
            variant="ghost"
            className="text-button-filter-text mb-2 bg-button-filter-background font-inter dark:bg-accent dark:text-button-filter-background font-medium flex gap-1 items-center px-3 h-8 text-sm rounded-[12px]"
            onClick={() => {
              setSelectedSubSection(null);
              table.getColumn("content_path")?.setFilterValue({
                ...filterValue,
                sub_section: null,
              });
            }}
            size="sm"
          >
            Sub-section: {selectedSubSection.subsection_name}
            <X className="transition-transform" />
          </Button>
        )}
        {selectedCollection && filterValue?.collection && (
          <Button
            variant="ghost"
            className="text-button-filter-text mb-2 bg-button-filter-background font-inter dark:bg-accent dark:text-button-filter-background font-medium flex gap-1 items-center px-3 h-8 text-sm rounded-[12px]"
            onClick={() => {
            ...
            size="sm"
          >
            Collection: {selectedCollection.collection_name.split("_")[0]}
            <X className="transition-transform" />
          </Button>
        )}
        {typeSelection &&
        ...
        {((filterDates.createdAt &&
          ...
          <Button
            ...
            onClick={() => {
              ...
              setSelectedCollection(null);
              setSelectedSubSection(null);
              table.getColumn("content_path")?.setFilterValue(undefined);
      ...
      {selectedRows.length > 0 && (
        <>
          {selectedRows.length === 1 && (
            <>
              <Link
                href={`/sections/${sectionID}/article/edit/${selectedRows[0].id}`}
              >
...
//   setSelectedSubSection: React.Dispatch<
//     React.SetStateAction<SubSection | null>
//   >;
//   options: number[];
// }) => {
...
//           >
//             {selectedSubSection ? (
//               selectedSubSection.subsection_name
//             ) : (
//               <span className="text-gray-500">Select Sub Section</span>
//             )}
...
//             <CommandList>
//               <CommandEmpty>No Sub section found.</CommandEmpty>
//               <CommandGroup>
//                 {subSections
//                   .filter((i) => i.id && options.includes(i.id))
//                   .map((sub_section, ix) => (
//                     <CommandItem
//                       value={String(sub_section.id)}
//                       key={ix}
//                       onSelect={() => {
//                         if (selectedSubSection?.id == sub_section.id) {
//                           setSelectedSubSection(null);
//                         } else {
...
//                         className={cn(
//                           "ml-auto",
//                           selectedSubSection?.id === sub_section?.id
//                             ? "opacity-100"
//                             : "opacity-0"
//                         )}
//                       />
...
//             ) : (
//               <span className="text-gray-500">Select Collection</span>
//             )}
//             <ChevronsUpDown className="opacity-50" />
//           </Button>
//         </PopoverTrigger>
...
//                   .map((_collection, ix) => (
//                     <CommandItem
//                       value={String(_collection.id)}
//                       key={ix}
//                       onSelect={() => {
//                         if (selectedCollection?.id == _collection.id) {
//                           setCollection(null);
//                         } else {
...
//                         className={cn(
//                           "ml-auto",
//                           selectedCollection?.id === _collection?.id
//                             ? "opacity-100"
//                             : "opacity-0"
//                         )}
//                       />
